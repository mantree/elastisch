...
(ns clojurewerkz.elastisch.rest-async
...
(defn put
  [^String uri fnc &{:keys [body] :as options}]
  (io! (httpkit/put uri
                    (merge options {:accept :json :body (json/encode body) :throw-exceptions throw-exceptions})
                    (fn [resp]
                      (fnc (json/decode (:body resp) true))))))

(defn get
  ([^String uri fnc]
     (io!
      (httpkit/get uri {:accept :json :throw-exceptions throw-exceptions}
                   (fn [resp]
                      (fnc (json/decode (:body resp) true))))))
  ([^String uri fnc &{:as options}]
     (io! (httpkit/get uri (merge options {:accept :json :throw-exceptions throw-exceptions})
                       (fn [resp]
                         (fnc (json/decode (:body resp) true)))))))
